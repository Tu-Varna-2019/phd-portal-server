...
public class DoctoralCenterServiceImpl implements DoctoralCenterService {
                ...
                @Override
                @Transactional
                public DoctoralCenter create(DoctoralCenterDTO doctoralCenterDTO)
                                                throws DoctoralCenterException {
                                LOG.info("Service received a request to create a doctor center user: "
                                                                + doctoralCenterDTO);
                                ...
                                doctoralCenter
                                                                .setRole(this.doctoralCenterRoleRepository
                                                                                                .getRoleByRole(doctoralCenterDTO.getRole()));

                                LOG.info("Check if user for the given email already exists: " + email);
                                if (doctoralCenterRepository.existsByEmail(email))
                                                throw new DoctoralCenterException("User with email: " + email
                                                                                + " already exists!");

                                LOG.info("Great, user with that email does not exist! Creating an autogenerated password for user now...");
                                doctoralCenter.generatePassword(16);
                                doctoralCenter.hashPassword();
                                doctoralCenter.setPasswordChangeRequired(true);
                ...
                public Uni<Void> sendEmail(String email, String password) {
                                String body = String.format("""
                                                                ...
                                                                """,
                                                                password,
                                                                this.clientBaseURL + "/change/password");
                ...
                @Override
                @Transactional
                // TODO: add the logic of checking whether the old password is equal to the new
                // one on the client side
                public void changePassword(DoctoralCenterPasswordChangeDTO dCenterPasswordChangeDTO)
                                                throws DoctoralCenterException {
                                LOG.info("Service received a request to change password for doctor center user: "
                                                                + dCenterPasswordChangeDTO);

                                String email = dCenterPasswordChangeDTO.getEmail();

                                LOG.info("Get user by email: " + email);
                                DoctoralCenter doctoralCenter = this.doctoralCenterRepository
                                                                .getDoctoralCenterByEmail(email);
                                LOG.info("Great, user with that email exist!");

                                doctoralCenter.setUnhashedPassword(dCenterPasswordChangeDTO.getOldPassword());
                                if (!doctoralCenter.doesPasswordMatch(doctoralCenter.getUnhashedPassword()))
                                                throw new DoctoralCenterException("Password do not match!");

                                LOG.info("Now changing password...");
                                doctoralCenter.setPassword(dCenterPasswordChangeDTO.getNewPassword());
                                doctoralCenter.hashPassword();
                                doctoralCenter.setPasswordChangeRequired(false);

                                LOG.info("Saved password to the database");
                };
}
