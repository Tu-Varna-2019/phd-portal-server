...
INSERT INTO "doctoralcenter" ("id", "oid", "name", "email", "DoctoralCenterRole")
				SELECT
								...
								dc_role.id
				FROM "doctoralcenterrole" dc_role
				WHERE dc_role.role = 'admin'
				AND NOT EXISTS (
								SELECT 1
								FROM "doctoralcenter"
...
WHERE NOT EXISTS (SELECT 1 FROM "mode" WHERE "type" = 'regular');

INSERT INTO "mode" ("id", "mode")
SELECT nextval('mode_id_seq'), 'part_time'
WHERE NOT EXISTS (SELECT 1 FROM "mode" WHERE "type" = 'part_time');
